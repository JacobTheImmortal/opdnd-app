...
// Lazily load the heavy DM tools (code-splitting)
const DmTools = lazy(() => import('./DmTools'));
...
export default function App() {
  ...
  /* ----------------------------------
     Hooks for DM screen (provided to DmTools)
  -----------------------------------*/
  const handleOpenSheetFromDm = (char) => {
    setCurrentChar(char);
  ...
  // DEV TOOLS — now a separate lazy‑loaded component
  if (step === 3) {
    return (
      <div style={{ padding: '1rem' }}>
        <button onClick={() => setStep(1)}>← Back</button>
        <Suspense fallback={<div>Loading DM Tools…</div>}>
          <DmTools
            charList={charList}
            setCharList={setCharList}
            onOpenSheet={handleOpenSheetFromDm}
          />
        </Suspense>
      </div>
    );
