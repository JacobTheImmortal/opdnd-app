...
/* Convert a desired modifier back into a raw ability score (nearest score with that mod) */
const scoreFromMod = (mod) => 10 + (Number(mod) * 2);
...
export default function App() {
  ...
  /* ----------------------------------
     Devil Fruit → Actions bridge (inline sample; you can wire your JSON like before)
  -----------------------------------*/
  ...
  if (step === 3) {
    ...
    const adjustMod = (char, statKey, delta) => {
      const updated = deepClone(char);
      // change modifier by ±1 => change underlying score by ±2
      updated.stats[statKey] = clamp((updated.stats[statKey] || 10) + delta * 2, 1, 30);
      // recalc derived since DEX/CON/INT/WIS influence things
      const derived = recalcDerived(updated);
      Object.assign(updated, derived);
      updated.currentHp = Math.min(updated.currentHp ?? derived.hp, derived.hp);
      updated.currentBar = Math.min(updated.currentBar ?? derived.bar, derived.bar);
      // also refresh meleeBonus from STR unless DM has custom values; we keep DM’s custom meleeBonus
      saveInline(updated);
    };
