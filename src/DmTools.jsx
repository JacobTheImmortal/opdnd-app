...
// --- BASE only (no DM mods) — matches App/Sheet math exactly
...
export default function DmTools({ charList, setCharList, onBack, onOpenSheet }) {
  ...
  const adminLevelAdjust = async (char, delta) => {
    ...
    updated.sp = Math.max(0, (updated.sp || 0) + (delta > 0 ? 3 : -3));
    // ensure mods present
  ...
  // --- UI helpers for DM modifiers (+1, +5, -5, Set…) ---
  ...
  return (
    <div style={{ padding: '1rem' }}>
      ...
      {/* Simple "Create Custom" helper (unchanged behavior) */}
      <div style={{ marginBottom: '0.75rem' }}>
        ...
        <button
          onClick={() => {
            alert('Create Random: keep your existing implementation if you had one.');
          }}
        >
          Create Random
        </button>
        ...
        return (
          <div key={char.id} style={{ border: '1px solid #ddd', marginBottom: '1rem', padding: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button style={{ marginLeft: 'auto' }} onClick={() => toggleExpand(char.id)}>
            ...
            {isOpen && (
              <div style={{ marginTop: '0.5rem' }}>
                {/* Core — show BASE and MODS separately; totals equal sheet */}
                <fieldset style={{ marginBottom: '0.75rem' }}>
                  ...
                  <div style={{ marginBottom: '0.4rem' }}>
                    ...
                    <span style={{ marginLeft: 8 }}>
                      = <strong>{totals.hp}</strong>
                    </span>
                    <span style={{ marginLeft: 8 }}>
                      <button onClick={() => bumpMod(char, 'hpMod', +1)}>+</button>
                      <button onClick={() => bumpMod(char, 'hpMod', +5)} style={{ marginLeft: 4 }}>
                        +5
                      </button>
                      <button onClick={() => bumpMod(char, 'hpMod', -5)} style={{ marginLeft: 4 }}>
                        -5
                      </button>
                      <button onClick={() => setModFromPrompt(char, 'hpMod', 'HP')} style={{ marginLeft: 4 }}>
                        Set…
                      </button>
                    </span>
                  </div>
                  ...
                  <div style={{ marginBottom: '0.4rem' }}>
                    ...
                    <span style={{ marginLeft: 8 }}>
                      = <strong>{totals.bar}</strong>
                    </span>
                    <span style={{ marginLeft: 8 }}>
                      <button onClick={() => bumpMod(char, 'barMod', +1)}>+</button>
                      <button onClick={() => bumpMod(char, 'barMod', +5)} style={{ marginLeft: 4 }}>
                        +5
                      </button>
                      <button onClick={() => bumpMod(char, 'barMod', -5)} style={{ marginLeft: 4 }}>
                        -5
                      </button>
                      <button onClick={() => setModFromPrompt(char, 'barMod', 'Bar')} style={{ marginLeft: 4 }}>
                        Set…
                      </button>
                    </span>
                  </div>
                  ...
                  <div style={{ marginBottom: '0.4rem' }}>
                    ...
                    <span style={{ marginLeft: 8 }}>
                      = <strong>{totals.reflex}</strong>
                    </span>
                    <span style={{ marginLeft: 8 }}>
                      <button onClick={() => bumpMod(char, 'reflexMod', +1)}>+</button>
                      <button onClick={() => bumpMod(char, 'reflexMod', +5)} style={{ marginLeft: 4 }}>
                        +5
                      </button>
                      <button onClick={() => bumpMod(char, 'reflexMod', -5)} style={{ marginLeft: 4 }}>
                        -5
                      </button>
                      <button onClick={() => setModFromPrompt(char, 'reflexMod', 'Reflex')} style={{ marginLeft: 4 }}>
                        Set…
                      </button>
                    </span>
                  </div>
                  ...
                  <div style={{ color: '#666', fontSize: 12, marginTop: 6 }}>
                    DM modifiers are flat additions that persist across level changes. Base is recalculated from race, stats, and
                    level; then the modifier is applied.
                  </div>
                </fieldset>

                {/* (Any additional DM panels you had remain unchanged) */}
              </div>
            )}
          </div>
        );
